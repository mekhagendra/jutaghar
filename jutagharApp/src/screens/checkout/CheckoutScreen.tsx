...
export default function CheckoutScreen({ onBack, onOrderSuccess }: CheckoutScreenProps) {
  ...
  const handleEsewaPayment = async (paymentData: { orderId: string; amount: number; taxAmount: number; shippingCost: number; total: number }, orderId: string) => {
    try {
      // Send only orderId — server derives all amounts from the stored Order
      const sigRes = await api.post<{ signature: string; signed_field_names: string; total_amount: string; transaction_uuid: string; product_code: string }>('/api/payment/esewa/signature', {
        orderId,
      });

      const { signature, signed_field_names, total_amount, transaction_uuid, product_code } = sigRes.data;
  ...
  const handleKhaltiPayment = async (paymentData: { orderId: string; total: number; orderNumber: string }, orderId: string) => {
    try {
      ...
      // Send only orderId + URLs — server derives amount and order name from the stored Order
